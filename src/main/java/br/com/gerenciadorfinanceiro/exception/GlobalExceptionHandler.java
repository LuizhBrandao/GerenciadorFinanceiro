...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ValidationError> validation(MethodArgumentNotValidException e, HttpServletRequest request) {
        ...
        ValidationError err = new ValidationError(Instant.now(), status.value(), "Validation Exception", "Erro de validação", request.getRequestURI());
    ...
    public ResponseEntity<StandardError> genericException(Exception e, HttpServletRequest request) {
        ...
        // Aqui também é recomendado fazer um e.printStackTrace() ou logar o erro
        e.printStackTrace();
        return ResponseEntity.status(status).body(err);
    }
}
